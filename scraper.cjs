 ...
 * scraper.cjs
 *
 * Production Playwright Product Matching & Pricing Scraper (CommonJS Version)
 * for KENT BUILDING SUPPLIES and THE HOME DEPOT
 *
 * Run:
 * node scraper.cjs [--competitor=kent|homedepot|all] [--limit=5] [--sku=...] [--search="..."]
 */
...
function calculateMatchScore(inventoryItem, candidate) {
  let score = 0;
  ...
  // HARD VETO: Treated vs Untreated
  ...
  // 4. Description String Similarity (+ up to 50)
...
async function scrapeSearchResults(page, config, searchTerm) {
...
async function findBestProductMatch(page, config, inventoryItem) {
  let allCandidates = [];
...
async function loadInventory(limit = 5, specificSku = null, searchQuery = null) {
  if (searchQuery) {
    return [{
      sku: "CLI-SEARCH",
      name: searchQuery,
