...
export const azureOAuthInit = (app: Hono) => {
  ...
  app.post('/make-server-8405be07/microsoft-oauth-init', async (c) => {
    try {
      ...
      // 1. Prioritize loading custom credentials from DB KV store first
      try {
        const config = await kv.get('secrets:microsoft');
        if (config && config.clientId) {
          AZURE_CLIENT_ID = config.clientId;
          AZURE_TENANT_ID = config.tenantId || 'common';
      ...
      if (!AZURE_CLIENT_ID) {
        ...
        AZURE_TENANT_ID = Deno.env.get('AZURE_TENANT_ID') || 'common';
      }
      if (!AZURE_REDIRECT_URI) {
        AZURE_REDIRECT_URI = Deno.env.get('AZURE_REDIRECT_URI') || '';
      }
      if (!AZURE_REDIRECT_URI) {
