...
export default async function handler(req, res) {
  ...
  try {
    ...
    await supabase.from('kv_store_8405be07').upsert({
      ...
      value: { action: 'reset', timestamp: nowIso }
    });
    ...
    try {
      ...
      fs.appendFileSync(logPath, `[${nowIso}] 🔄 Pricing agent status manually reset to item 0.\n`);
    } catch (e) {}

    return res.status(200).json({
      success: true,
      message: 'Pricing agent status reset successfully.',
      status: resetStatus
    });
  } catch (err) {
    return res.status(500).json({ error: err.message || 'Failed to reset pricing agent.' });
  }
}
